...
gulp.task('templatecache:inject',['templatecache:compile'], function(done) {
    helper.log('Wiring the templatecache dependencies into html');

    var target = gulp.src(templatecacheConfig.injectScr);
    var sources = gulp.src(templatecacheConfig.injectJsSrc)

    return target
        .pipe($.inject(sources, {name:"", ignorePath: 'src', addRootSlash: false}))
